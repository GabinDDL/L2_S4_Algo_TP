...
def puissance_matrice_2_2(M, n):
    ...
    if n % 2 == 0:
        ...
        return produit_matrice_2_2(carre, M)
...
# Les courbes entre la fonction normalisés et fibo4 croient de la même façon malgré les quelques différences
# croient à peu près de la même façon
...
def produit_matrice_2_2_ops(M1, M2):
    ...
    if (M1[0][0] == 1 and M1[0][1] == 0 and M1[1][0] == 0 and M1[1][1] == 1):
        return M2.copy(), 0

    if (M2[0][0] == 1 and M2[0][1] == 0 and M2[1][0] == 0 and M2[1][1] == 1):
        return M1.copy(), 0
...
def puissance_matrice_2_2_ops(M, n):
    ...
    nb_ops = 0
    if n == 0:
        return [[1, 0], [0, 1]], nb_ops
    tmp, nb_ops = puissance_matrice_2_2_ops(M, n//2)
    produit_tmps = produit_matrice_2_2_ops(tmp, tmp)

    carre = produit_tmps[0]
    nb_ops += produit_tmps[1]

    if n % 2 == 0:
        return carre, nb_ops
    else:
        produit_carre_M = produit_matrice_2_2_ops(carre, M)
        nb_ops += produit_carre_M[1]
        return produit_carre_M[0], nb_ops
...
def fibo_4_ops(n):
    ...
    if n <= 2:
        return 1, 0
    M = [[1, 1], [1, 0]]
    N, nb_ops = puissance_matrice_2_2_ops(M, n-1)
    return N[0][0], nb_ops

# COMMENTAIRES - question 2.3
# Les courbes obtenus suivent les mêmes évolutions que dans les premières questions, c'est à dire qu'ils croient de la même façon et les courbes se chevauchent
###############################################################################
#
# NE PAS MODIFIER !!!
#
...
def decompte_naif(a, b, c, d, e):
    ...
    return nbOfBits(a) + nbOfBits(b)*nbOfBits(c) + nbOfBits(d)*nbOfBits(e)
...
def decompte_karatsuba(a, b, c, d, e):
    ...
    return nbOfBits(a) + nbOfBits(max(b, c)**log2(3)) + nbOfBits(max(d, e)**log2(3))
# À COMPLÉTER
#
...
def produit_matrice_2_2_bits(M1, M2, compte=decompte_naif):
    ...
    res[0][0] = M1[0][0] * M2[0][0] + M1[0][1] * M2[1][0]
    nb_bits = compte(res[0][0], M2[0][0], M1[0][1], M2[1][0])
    res[1][0] = M1[1][0] * M2[0][0] + M1[1][1] * M2[1][0]
    nb_bits += compte(res[1][0], M1[1][0], M2[0][0], M1[1][1], M2[1][0])
    res[0][1] = M1[0][0] * M2[0][1] + M1[0][1] * M2[1][1]
    nb_bits += compte(res[0][1], M1[0][0], M2[0][1], M1[0][1], M2[1][1])
    res[1][1] = M1[1][0] * M2[0][1] + M1[1][1] * M2[1][1]
    nb_bits += compte(res[1][1], M1[1][0], M2[0][1], M1[1][1], M2[1][1])
...
def courbes_temps(n, tous=True, pas=1):
    ...
    if tous:
        temps += [[]]
        temps[0] = [tp2_ex1.mesure(tp2_ex1.fibo_3, i)
...
def test_prod22():
    ...
    printcol('{** Score %d/%d : %s}' % (score, ldata,
             "super !" if score == ldata else "essaie encore !"), 'bold')
    print()
...
def test_puissanceData():
    return [[[[1, 4], [2, 1]], 2, [[9, 8], [4, 9]]],
            ...
            [[[3, 12], [17, 26]], 8, [[252018687525, 442324154508],
                                      [626625885553, 1099806650332]]],
            ...
            [[[60, 5], [4, 1]], 7, [[2894868940020, 243934318805],
                                    [195147455044, 16443978121]]],
            [[[54, 30], [11, 4]], 10, [[538139813380894176, 288789026527531200], [
            ...
            [[[60, 5], [4, 1]], 9, [[10538945536660820, 888057647401005],
                                    [710446117920804, 59865297328961]]]
            ]
...
if __name__ == '__main__':
    ...
    # fibo_3 et fibo_4, mltiplication naïve
    courbes_bits(10000, pas=10)
    # fibo_3 et fibo_4, mltiplication par Karatsuba
    courbes_bits(10000, pas=10, compte=decompte_karatsuba)
    # seulement fibo_4, mltiplication par Karatsuba
    courbes_bits(10000, tous=False, pas=10, compte=decompte_karatsuba)
