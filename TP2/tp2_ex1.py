...
# On voit sur les deux graphiques que les courbe du temps mis pour les fonctions normalises et la
# fonction testée sont proche et croient de la même façon.
...
def fibo_1_adds(n):
    ...
    fb1 = fibo_1_adds(n-1)
    fb2 = fibo_1_adds(n-2)
    return fb1[0] + fb2[0], 1 + fb1[1] + fb2[1]
...
# On a une bonne représentation : la courbe de fibo_1_adds a une allure plus
# exponentielle quand les deux autres sont plus linéaire, comme le temps mis
# pour les différents fibo
...
def courbes_adds(n, tous=True, pas=1):
    ...
    algos = [fibo_1_adds, fibo_2_adds, fibo_3_adds] if tous else [
        fibo_2_adds, fibo_3_adds]
    nb_ops = [[algo(i)[1] for i in range(0, n, pas)] for algo in algos]

    l = ['fibo_1_adds', 'fibo_2_adds', 'fibo_3_adds'] if tous else [
        'fibo_2_adds', 'fibo_3_adds']
...
def fibo_1_bits(n):
    ...
    return op, nbOfBits(op) + fb1[1] + fb2[1]
...
# Oui encore une fois les courbes ont les mêmes allures que celle donnés précédemment
...
def courbes_ops(n, tous=True, pas=1):
    ...
    algos = [fibo_1_bits, fibo_2_bits, fibo_3_bits] if tous else [
        fibo_2_bits, fibo_3_bits]
    nb_ops = [[algo(i)[1] for i in range(0, n, pas)] for algo in algos]
    ...
    l = ['fibo_1_bits', 'fibo_2_bits', 'fibo_3_bits'] if tous else [
        'fibo_2_bits', 'fibo_3_bits']
    for valeurs, couleur in zip(nb_ops, colors(tous)):
        if couleur != 'black':
            plot(range(0, n, pas), valeurs, couleur)
...
if __name__ == '__main__':
    ...
    # seulement fibo_2_adds et fibo_3_adds
    ...
    # seulement fibo_2_bits et fibo_3_bits
